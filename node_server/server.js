...
app.post('/api/make-call', async (req, res) => {
    ...
    try {
        ...
        const voiceUrl = `${process.env.BASE_URL}/node/api/voice/twiml-connect?leadId=${leadId}&callId=${callId}`;
...
app.post('/api/voice/twiml-connect', (req, res) => {
    ...
    baseUrlObj.protocol = 'wss:';
    const wsUrl = `${baseUrlObj.toString().replace(/\/+$/, '')}/media?leadId=${leadId}&callId=${callId}`;
...
server.on('upgrade', (req, socket, head) => {
    ...
    if (req.url && req.url.startsWith('/media')) {
        ...
        // reject everything that�s not /media...
